...
class Car {
	...
	Human owner;/*
		소유주 : Human클래스를 기반으로 하는 멤버변수로 
			초기화를 위해 이름, 나이, 에너지 세 가지의 값이 필요하다.
			클래스를 구성할 때 또 다른 객체를 멤버변수로 선언할 수 있다.  
			*/
...
public class E03CarMain {
	...
	public static void main(String[] args) {
		...
		// car1.drive();  <- 에러발생
				/*(자동차 소유주를 표현하는 Human 객체가 초기화되지 않은 
		  			상태에서 출력을 시도하여 에러가 발생된다. )
		 		*/
		...
		//자동차2 객체 생성 : 객체 생성 후 멤버변수를 직접 접근하여 초기화한다. 
		Car car2 = new Car();
		...
		//세 번째  자동차 객체 생성 : 매개변수가 있는 초기화 메소드를 이용하여 초기화
		Car car3 = new Car();
