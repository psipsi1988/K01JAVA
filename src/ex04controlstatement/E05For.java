...
public class E05For {
	...
	public static void main(String[] args) {
		...
		2. 조건을 검사(i가 5 이하이면 참)
		3. 참일 때 실행문장을 실행
		4. 증가(감소)식 실행
		5. 조건 검사(3, 4, 5번을 반복 횟수만큼 실행함)
		6. 조건이 거짓일 때 for문 종료
		 */
		for(int i=1 ; i<=5 ; i++) {
		...
		total = 0;
		for(int i=0 ; i<=10 ; i+=2) {//증감연산자 대신 복합대입연산자도 사용 가능. 
			total += i;
...
//	
		...
		여기서 사용된 변수j는 지역 변수(Local variable)라고 한다. 
		지역 변수는 해당 지역을 벗어나면 소멸되는 특징을 가지고 있다. 
		 */
		//System.out.println("위 for문에서 선언한 변수 j="+ j);//좁은 지역에서 선언해서 못 씀
		System.out.println("위 main함수 지역에서 선언한 변수 total="+total);
		
		
		/*
		for문의 초기값을 외부 (넓은 지역)에서 선언하면 해당 변수는 for문 안쪽 (좁은 지역)에서 
		사용이 가능하다. 
		즉, 아래에서 선언한 i는 main 함수의 지역변수가 된다. 
		 */
		int i=0;
		for( ; i<=5 ; i++) {
		...
		System.out.println("==감소 연산자 사용으로 구현==");
		for (int x = 1; x<=4 ; x++) {
			for(int y=4 ; y>=1 ; y--) {
				if(x==y) {
					System.out.print("1 ");
		...
		for (int dan = 2; dan <= 9 ; dan++) {
			for(int su = 1 ; su <= 9 ; su++) {
				...
				System.out.printf("%d" + "×%d" + "= %-4d", //간단하게 하는 방법 해보기
						dan, su, (dan*su));
			}
			System.out.println();
		}
