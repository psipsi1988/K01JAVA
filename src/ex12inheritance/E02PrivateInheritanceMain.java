...
class Account{
	...
	//private멤버 변수를 외부클래스로 반환할 때 사용함
	protected int getAccMoney() {
		return money;
	}
}

class SavingAccoun extends Account{
	
	/*
	인자생성자에서 부모의 생성자 메소드를 호출하기 위해 super()를 사용하고 있다. 
	이때 인자가 하나인 생성자를 호출하게 된다. 
	 */
	public SavingAccoun(int initVal) {
		super(initVal);
	}
	...
	public void showAccountMoney() {
		...
		//System.out.println("지금까지의 누적 금액:" + money);//[접근 불가]
	}
}
...
public class E02PrivateInheritanceMain {
	...
	public static void main(String[] args) {
		...
		/*
		private 멤버이므로 상속받은 하위클래스에서의 참조변수를 통해서	접근이 불가능하다.
		부모의 멤버메소드를 통해 접근한다.  
		 */
		SavingAccoun sa = new SavingAccoun(10000);
