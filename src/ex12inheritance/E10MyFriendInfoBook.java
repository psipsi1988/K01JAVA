...
/*
친구를 표현한 최상위 클래스로 해당 프로그램에서는 Friend로 객체 생성은 하지 않는다. 
상속의 목적으로만 정의된 클래스이다. 
 */
...
class FriendInfoHandler{
	...
	하나의 배열 내에 두 개의 객체를 동시에 저장하여 관리할 수 있다.  
	 */
	private Friend[] myFriends;
	private int numOfFriends;//친구정보를 추가할 때마다 +1 증가
	
	//생성자 : 인자로 전달되는 num 크기로 객체 배열을 생성한다. 
	public  FriendInfoHandler(int num) {
		...
		public void addFriend(int choice) {
			...
			/*
			1. 친구 정보를 입력받은 후...
			2. 객체배열 0번 방에 객체를 저장하고... 
			3. numOfFriends 변수를 1 증가시킨다. (후위 증가)
			 */
			if(choice==1) {
		...
		3. 객체배열에 저장된 객체들을 for문을 통해 순차적으로 출력. 
			이때 오버라이딩된 메소드를 통해 항상 해당 객체의 정보를 출력할 수 있다. 
			(참조변수의 타입에 영향을 받지 않음)
		 */
		...
		public void searchInfo() {
			...
			for (int i=0 ; i<numOfFriends ; i++) {
				System.out.println("검색 중인 이름:" + myFriends[i].name);
