...
/*
컴퓨터를 상속한 노트북 컴퓨터
	:컴퓨터가 가지고 있는 기본 기능에 휴대 이용 부분을 확장하여 정의하였음.
	휴대 이용을 위해 배터리 멤버변수가 추가됨.  
 */
class NotebookComputer extends Computer{
	...
	앞에 다른 문장이 먼저 오게 되면 오류가 발생되므로 주의해야 한다. 
	 */
	...
	public void movigCal() {
		...
		/*
		아래 메소드는 private으로 선언되었으므로 상속받은 클래스에서도 
		접근은 불가능하다. 
		 */
		//keyboardTouch();//에러발생
		//calculate();//에러발생
...
/*
태블릿 노트북 컴퓨터
	: 컴퓨터의 기본 기능, 휴대 이용에 펜으로 드로잉할 수 있는 기능까지 추가된 클래스
 */
class TabletNotebook extends NotebookComputer{
	...
	public void  write(String penInfo) {
		...
		A.compare To(B)
			...
			판단하고, -1 혹은 1이 반환되면 서로 다른 문자열로 판단하는 
			String 클래스에 정의된 메소드
		 */
