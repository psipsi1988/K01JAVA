...
instanceof 연산자
	...
	형변환(장속관계가 있으면)이 가능하면 true, 불가능하면 false를 반환한다. 
...
public class E09InstanceOf {
	...
	/*
	-여기에서 instanceof 연산자를 통해 형변환 가능 여부를 판단하는 이유는 
	전달된 매개변수가 최상위클래스인 BOX타입으로 받고 있기 때문이다. 
	-Box타입으로 인자를 받게 되면 자동으로 업캐스팅(자동형변환)이 되기 때문에 
	자식클래스의 멤버메스드를 호출할 수 없게 된다. 
