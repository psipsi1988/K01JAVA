...
/*
2] 런타임에러
-컴파일 시에는 체크가 안 되고 실행 시에만 발생되는 에러
-RuntimeWxecption 클래스 계열로 NullPointerWexeption이 대표적인 하위 클래스이다. 
-main메소드에서 무시하기 위해 throws하더라도 로유가 발생되므로 
반드시 예외 처리를 해줘야 한다. 
-예외 발생시 JVM은 해당 예외 클래스를 인스턴스화해서
 프로그램 쪽으로 전달하게 되고 try~catch에서 받은 후 처리한다. 
 */
public class Ex02PreDefineException {
	...
	public static void main(String[] args) {
		...
		try {
			...
			e.printStackTrace();//getMessage보다 더 많이 쓰임
		}
		...
		: 숫자 형식의 문자열을 int로 변환하는 경우 
		해당 문자열이 숫자 형식이 아니면 발생되는 예외
		 */
		...
		try {
			...
			int ageAfter10 = Integer.parseInt(strAge) + 10;// 예외 발생
		...
		: 숫자를 입력해야 하는데 문자열을 입력할 때 발생되는 예외
		 */
		...
		try {
			...
			System.out.println("나이를 문자 형태로 입력하면 안 돼요. ");
			System.out.println("예외메시지 : " + e.getMessage());
			e.printStackTrace();
		}
		...
		4] NullPointerException
			...
			발생되는 예외. 
			즉, 참조변수만 생성되고 참조할 객체가 없는 상태에서 사용했을 때 발생하게 된다. 
		 */
		...
		try {
			/*
			참조변수가 참조할 객체가 없는 상태이므로 해당 메소드를 호출할 수 없어 예외가 발생된다. 
			 */
